...
# Demo for scraping specific books from Travel category (11 books total)
# Bad books: < 3 stars rating
# Cheap books: < £40.00
...
url_to_crawl = "https://books.toscrape.com/catalogue/category/books/travel_2/index.html"
log_path = "./logs/travel_books.txt"
...
def get_books():
    ...
    try:
        while True:
            ...
            for book in books:
                ...
                if info['rating'] < 3:
                    bad_books_list.append(info)

                if float(info['price'][1:]) < 40.00:
                    cheap_books_list.append(info)

                if ':' in info['title']:
                    colon_books_list.append(info)
    ...
    return bad_books_list, cheap_books_list, colon_books_list

if __name__ == "__main__":
    bad_books_list, cheap_books_list, colon_books_list = get_books()

    # Dump data in JSON format
    with open("./data/bad_books.json", "w", encoding="utf-8") as file:
        json.dump(bad_books_list, file, ensure_ascii=False, indent=4)
    with open("./data/cheap_books.json", "w", encoding="utf-8") as file:
        json.dump(cheap_books_list, file, ensure_ascii=False, indent=4)  
    with open("./data/colon_books.json", "w", encoding="utf-8") as file:
        json.dump(colon_books_list, file, ensure_ascii=False, indent=4) 
